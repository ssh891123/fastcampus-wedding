...
export default function Share({ gromeName, brideName, date }: ShareProp) {
  useEffect(() => {
  ...
  const handleShareKakao = () => {
    window.Kakao.Share.sendDefault({
      objectType: 'feed',
      content: {
        title: `${gromeName} ♥ ${brideName} 결혼합니다.`,
        description: `${format(parseISO(date), 'M월 d일 eeee aaa h시', { locale: ko })}`,
        imageUrl:
          'https://img.freepik.com/premium-vector/beautiful-bride-and-wonderful-groom-two-people-having-a-wedding_870049-176.jpg',
        link: {
          mobileWebUrl: window.location.origin, //도메인, URL, 포트번호까지 origin에 포함
